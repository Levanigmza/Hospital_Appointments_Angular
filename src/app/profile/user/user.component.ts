...
import { UserDetailsEditComponent } from './user-detailsEdit/user-details-edit.component';
...
@Component({
  ...
  viewProviders: [UserAuthService, LogoutService, AppointmentService, UpdateUserDataService, GetDoctorsService, AppointmentService, OtpService],
...
export class UserPageComponent {
  ...
  async getUserAppointments(): Promise<void> {
    try {
      let appointmentsResponse: IAppointment[] | undefined = await this.appointmentService.getUserAppointments(this.UserAuthService.getAuthToken() ?? '').toPromise();
  ...
  currentUserReservation(appointment: IAppointment): boolean {
    ...
    return appointment.userId === this.user.userId.toString() && appointment.comment === "ჩემი ჯავშანი";
  }
